...
public class TP2Q18 {
    ...
    public static void main(String args[]) throws Exception {
       ...
       for(int i=0;i<27;i++) {
            ...
            try {
                vetor[pos++].ler();
                QuicksortParcial(vetor, 0, pos-1);
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter("matricula_quicksortparcial.txt"))) {
    ...
    //ordenação por qucick sort parcial
    public static void QuicksortParcial(Personagem[] vetor, int esq, int dir){
        int i = esq;
        int j = dir;
        int pivo = esq;
        int k = 10;
        while(i <= j){
            while(vetor[i].getCasa().compareTo(vetor[pivo].getCasa()) < 0 || (vetor[i].getCasa().compareTo(vetor[pivo].getCasa())==0 
            && vetor[i].getNome().compareTo(vetor[pivo].getNome())<0)){
                i++;
            }
            while(vetor[j].getCasa().compareTo(vetor[pivo].getCasa()) > 0 || (vetor[j].getCasa().compareTo(vetor[pivo].getCasa())==0 
            && vetor[j].getNome().compareTo(vetor[pivo].getNome())>0)){
                j--;
            }
            if(i <= j){
                //swap das informações dos vetores
        ...
        if(esq < j){
            QuicksortParcial(vetor, esq, j);
        }
        if(i < k && dir > i)
        QuicksortParcial(vetor, i, dir);	    
    }
}
