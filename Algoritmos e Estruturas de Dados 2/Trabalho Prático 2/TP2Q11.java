...
public class TP2Q11 {
  ...
  //counting sort
    public static void counting(Personagem[] vetor, int pos) {
        //Vetor para contar o número de ocorrências de cada elemento
        int[] count = new int[getMaior(vetor, pos) + 1];
        Personagem[] ordenado = new Personagem[pos];
    
        // Inicializar cada posição do array de contagem
        for (int i = 0; i < count.length; count[i] = 0, i++);
    
        // Agora, o count[i] contém o número de elementos iguais a i
        for (int i = 0; i < pos; count[vetor[i].getAnoNascimento()]++, i++);
    
        // Agora, o count[i] contém o número de elemento menores ou iguais a i
        for(int i = 1; i < count.length; count[i] += count[i-1], i++);
    
        // Ordenando
        for(int i = pos-1; i >= 0; ordenado[count[vetor[i].getAnoNascimento()]-1] = vetor[i],mov++, count[vetor[i].getAnoNascimento()]--, i--);
    
        // Copiando os elementos ordenados de volta para o vetor original
        for(int i = 0; i < pos; vetor[i] = ordenado[i],mov++, i++);
    }
    
    public static int getMaior(Personagem vetor[], int n) {
        int maior = vetor[0].getAnoNascimento();
 
         for (int i = 0; i < n; i++) {
          if(maior < vetor[i].getAnoNascimento()){
             maior = vetor[i].getAnoNascimento();
             mov++;
          }
         }
        return maior;	
  ...
  //metódo para realizar a segunda ordenação(por nome)
    private static Personagem[] ordenarNome(Personagem[] vetor, int pos){
        for(int i = 1; i < pos; i++){
            ...
            {
                comp++;
                while((j >= 0) && (vetor[j].getAnoNascimento() == temp.getAnoNascimento()) && vetor[j].getNome().compareTo(temp.getNome()) > 0){
                    comp++;
                    vetor[j + 1] = vetor[j];
                    mov++;
                    j--;
