...
void bubbleSort(Personagem vetor[], int pos){
    int i, j;
    for (i = 0; i < (pos-1); i++) {
        for (j = 0; j < (pos-1-i); j++) {
            comp++;
            if (strcmp(vetor[j].corCabelo, vetor[j+1].corCabelo) > 0 || 
               (strcmp(vetor[j].corCabelo, vetor[j+1].corCabelo) == 0 && strcmp(vetor[j].nome, vetor[j+1].nome) > 0)) {
