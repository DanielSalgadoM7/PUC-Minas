...
//Atributo para ordenação = cor do olho
...
void insercaoPorCor(Personagem vetor[], int pos, int cor, int h) {
    for(int i = (h + cor); i < pos; i += h) {
        Personagem tmp = vetor[i];
        int j = i - h;
        while((j >= 0) && (strcmp(vetor[j].corOlho, tmp.corOlho) || (strcmp(vetor[j].corOlho, tmp.corOlho) && strcmp(vetor[j].nome, tmp.nome) > 0))) {
            strcpy(vetor[j + h].corOlho, vetor[j].corOlho);
            vetor[j + h] = vetor[j];
            j -= h;
            comp+=2;
            mov++;
...
void shellsort(Personagem vetor[], int pos){
    int h = 1;
    do{
        h=(h*3) + 1;
    }while(h < pos);
    ...
    do{
        ...
        for(int cor = 0; cor < h; cor++){
            insercaoPorCor(vetor, pos, cor, h);
        }
    }while(h != 1);
}
