...
public class TP2Q09 {
    ...
    public static void heapSort(Personagem vetor[], int pos) {
        // Construção do heap
        for (int i = pos / 2 - 1; i >= 0; i--) {
            construir(pos, i, vetor);
        }
        ...
        for (int i = pos - 1; i > 0; i--) {
            ...
            // Chama o método construir na subárvore reduzida 
            construir(i, 0, vetor);
        }
    }

    //fazendo o metódo desse jeito, não é necessário possuir o metódo de reconstruir
    public static void construir(int tam, int i, Personagem[] vetor) {
        ...
        if (esq < tam && (vetor[esq].getCorCabelo().compareTo(vetor[maior].getCorCabelo()) > 0 ||
                          (vetor[esq].getCorCabelo().equals(vetor[maior].getCorCabelo()) &&
                           vetor[esq].getNome().compareTo(vetor[maior].getNome()) > 0))) {
            maior = esq;
        }

        // Se o filho da direita é maior que o maior até agora
        if (dir < tam && (vetor[dir].getCorCabelo().compareTo(vetor[maior].getCorCabelo()) > 0 ||
                          (vetor[dir].getCorCabelo().equals(vetor[maior].getCorCabelo()) &&
                           vetor[dir].getNome().compareTo(vetor[maior].getNome()) > 0))) {
            maior = dir;
        }
        ...
        if (maior != i) {
            ...
            construir(tam, maior, vetor);
        }
    }

    //metódo para trocar os personagens
    public static void swap(Personagem[] vetor, int a, int b) {
        Personagem temp = vetor[a];
        vetor[a] = vetor[b];
        vetor[b] = temp;
        mov += 3;
    }
