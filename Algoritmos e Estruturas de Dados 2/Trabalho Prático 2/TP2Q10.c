...
void quicksort(Personagem vetor[], int esq, int dir) {
    int i = esq, j = dir;
    Personagem pivo = vetor[(esq + dir) / 2];

    while (i <= j) {
        while (strcmp(vetor[i].casa, pivo.casa) < 0 || 
               (strcmp(vetor[i].casa, pivo.casa) == 0 && strcmp(vetor[i].nome, pivo.nome) < 0)) {
            i++;
            comp++;
        }
        while (strcmp(vetor[j].casa, pivo.casa) > 0 || 
               (strcmp(vetor[j].casa, pivo.casa) == 0 && strcmp(vetor[j].nome, pivo.nome) > 0)) {
            j--;
            comp++;
        }
        if (i <= j) {
