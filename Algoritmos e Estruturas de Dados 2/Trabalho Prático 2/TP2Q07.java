/*Repita a questão de Ordenação por Seleção, contudo, usando o algoritmo de Inserção, fazendo com que a chave de pesquisa seja o 
atributo dateOfBirth. O nome do arquivo de log será matrícula_insercao.txt.
...
public class TP2Q07 {
    ...
    public static void Insercao(Personagem[] vetor, int pos){
    for(int i = 1; i < pos; i++){
        Personagem chave = vetor[i];
        LocalDate tmpData = chave.getDataNascimento();
        String tmpNome = chave.getNome();
        int j = i - 1;
        // Ordena primeiro por data de nascimento e dps compara com os nomes
        while(j >= 0 && (vetor[j].getDataNascimento().isAfter(tmpData) || 
                        (vetor[j].getDataNascimento().isEqual(tmpData) && vetor[j].getNome().compareTo(tmpNome) > 0))){
            vetor[j + 1] = vetor[j];
            j--;
        }
        vetor[j + 1] = chave;
