...
desconsiderados pois são menores ou iguais a X. Como o valor 30 está dentro da especificação (maior do que X) ele será válido e então deve-se processar os cálculos para apresentar na saída 
...
int main() {
  int valorX = 0, valorZ = 0, contador = 1, parada = 0;
  scanf("%d %d", &valorX, &valorZ);
  while (valorX >= valorZ) {
    scanf("%d", &valorZ);
  }
